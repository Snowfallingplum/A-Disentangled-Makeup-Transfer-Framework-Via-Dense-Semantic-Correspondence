...
from  options import SemanticOptions
parser = SemanticOptions()
opts = parser.parse()

# code from https://github.com/cvlab-yonsei/SFNet
...
class SFNet(nn.Module):
    ...
    def forward(self, src_img, tgt_img, GT_src_mask=None, GT_tgt_mask=None, train=True):
        ...
        if not train:
            ...
            warped_img_S2T = F.grid_sample(src_img, grid_S2T, mode='bilinear')
            warped_img_T2S = F.grid_sample(tgt_img, grid_T2S, mode='bilinear')
            ...
            return {'est_src_mask': warped_src_mask, 'smoothness_S2T': smoothness_S2T, 'grid_S2T': grid_S2T,
                    ...
                    'warped_img_S2T': warped_img_S2T, 'warped_img_T2S': warped_img_T2S}

    def visualization(self,src_img, tgt_img, GT_src_mask, GT_tgt_mask,grid_S2T,grid_T2S):
        grid_S2T1 = grid_S2T.permute(0, 3, 1, 2)
        grid_T2S1 = grid_T2S.permute(0, 3, 1, 2)
        grid_S2T1 = F.interpolate(grid_S2T1, scale_factor=16, mode='bilinear', align_corners=True)
        grid_T2S1 = F.interpolate(grid_T2S1, scale_factor=16, mode='bilinear', align_corners=True)
        grid_S2T1 = grid_S2T1.permute(0, 2, 3, 1)
        grid_T2S1 = grid_T2S1.permute(0, 2, 3, 1)

        GT_src_mask=GT_src_mask.permute(0, 3, 1, 2)
        GT_tgt_mask=GT_tgt_mask.permute(0,3,1,2)
        GT_src_mask = F.interpolate(GT_src_mask, scale_factor=16, mode='nearest')
        GT_tgt_mask = F.interpolate(GT_tgt_mask, scale_factor=16, mode='nearest')
        GT_src_mask = GT_src_mask.permute(0, 2, 3, 1)
        GT_tgt_mask = GT_tgt_mask.permute(0, 2, 3, 1)
        warped_img_S2T = F.grid_sample(tgt_img, grid_S2T1, mode='bilinear')
        warped_img_T2S = F.grid_sample(src_img, grid_T2S1, mode='bilinear')
        warped_mask_S2T = F.grid_sample(GT_tgt_mask, grid_S2T1, mode='bilinear')
        warped_mask_T2S = F.grid_sample(GT_src_mask, grid_T2S1, mode='bilinear')
        row1 = torch.cat(
            (src_img[0, ::], tgt_img[0, ::], warped_img_S2T[0, ::],warped_img_T2S[0, ::]), 2)
        row2 = torch.cat(
            (GT_src_mask[0, ::], GT_tgt_mask[0, ::], warped_mask_S2T[0, ::],warped_mask_T2S[0, ::]), 2)
        return row1,row2
